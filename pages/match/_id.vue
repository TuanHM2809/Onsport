      ...
      <div class="row">
        ...
        <div class="col-sm-8">
          ...
          <div v-if="!isMobile" v-loading="loading" style="background: #fff;">
            <h1 class="fixtures-title p-l-15">Lịch thi đấu</h1>
...
export default {
  ...
  async fetch ({store, params, error}) {
    try {
      await Promise.all([
        store.dispatch('loadMatchEvents', params),
        store.dispatch('loadMatchStats', params)
      ])
      // console.log(store.state.pundit.detail.data)
      // if (Object.keys(store.state.match.events).length === 0) {
  ...
  head () {
    return {
      title: this.matchDetail.home.name + ' vs ' + this.matchDetail.away.name + ' | ON Sports - Trang tin tức bóng đá nhanh và chính xác nhất',
      meta: [
        { hid: 'keywords',
          name: 'keywords',
          content: this.matchDetail.home.name + ', ' + this.matchDetail.away.name
        },
        { hid: 'description', name: 'description', content: 'Match ' + this.matchDetail.home.name + ' - ' + this.matchDetail.away.name },
        { hid: 'og:title', property: 'og:title', content: 'Match ' + this.matchDetail.home.name + ' - ' + this.matchDetail.away.name },
        { hid: 'og:type', property: 'og:type', content: 'Match ' + this.matchDetail.home.name + ' - ' + this.matchDetail.away.name },
        { hid: 'og:description', property: 'og:description', content: 'Match ' + this.matchDetail.home.name + ' - ' + this.matchDetail.away.name },
        { hid: 'og:url', property: 'og:url', content: 'match' }
      ]
    }
  },
  computed: {
    ...
    events () {
      return this.$store.state.match.events
    },
    reverseEvents () {
      return this.$store.getters.reverseEvents
    },
    stats () {
      return this.$store.state.match.stats
    },
    listStat () {
      return _.map(this.$store.state.match.stats.home, (obj) => (obj.type.name))
    },
    // homeGoals () {
    //   const {home} = this.$store.state.match.info
    //   const homeEvents = _.filter(this.$store.state.match.events, event => {
    //     return event.action_owner_team.id === home.id
    //   })
    //   // return home
    ...
    // },
    homeGoals () {
      const {home, away} = this.$store.state.match.info
    ...
    awayGoals () {
      const {home, away} = this.$store.state.match.info
      const awayEvents = _.filter(this.$store.state.match.events, event => {
  ...
  methods: {
    ...
    toTitleCase (str) {
      ...
      // }).join(' ') Quá khứ rồi! giờ cần việt hóa
